...
provider "proxmox" { 
    ...
    pm_user         = "root@pam"                               #Logga in som denna anvandaren
    pm_password     = "root@pam"
    pm_tls_insecure = true                                     #Ignorera sakerhetsvarningar (certifikat)
}
...
resource "proxmox_vm_qemu" "webserver" {            
    name        = "test-server/vm?"                 #Vilken fysisk server i PRoxmox den ska koras pa
    target_node = "proxmox"

    clone "ubuntu/ubuntu_noble_template" {           #Kopiera en fardig VM (template)
    
    cores  = 1                                      #Ge VM:en CPU
    memory = 1                                      #Ge VM:en 1 GB RAM
    }

    network {                                        #Konfigurering av network
        model  = "virtio"                           #Vilken typ av natverkskort VM:en ska ha. virto = snabb (standard)
        bridge = "vmbr0"                            #Koppla VM:en till natverk vmbr0 (vmbr0 = ditt natverk i proxmox)
        
    }
}

provisioning "remote-exec" {                         #Kor kommandon inne i VM:en
